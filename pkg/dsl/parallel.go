...
func ApplyFunctionToInputList[T any](ctx context.Context, workers int, rtx *Runtime, f0 Function, input []T) []any {
...
func ApplyInputToFunctionList[T any](ctx context.Context, workers int, rtx *Runtime, fs []Function, input T) []any {
	// spawn goroutine for distributing functions to workers
	src := make(chan Function)
	go func() {
		defer close(src)
		for _, f := range fs {
			src <- f
