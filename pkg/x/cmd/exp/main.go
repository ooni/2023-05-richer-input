package main

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/apex/log"
	"github.com/ooni/2023-05-richer-input/pkg/ric"
	"github.com/ooni/2023-05-richer-input/pkg/ril"
	"github.com/ooni/2023-05-richer-input/pkg/rix"
	"github.com/ooni/probe-engine/pkg/runtimex"
)

func main() {
	pipeline := ril.Compose(
		ril.DomainName("www.example.com"),
		ril.DNSLookupParallel(
			ril.DNSLookupUDP("8.8.8.8:53"),
			ril.DNSLookupUDP("[2001:4860:4860::8844]:53"),
			ril.DNSLookupGetaddrinfo(),
			ril.DNSLookupStatic("10.10.34.34", "10.10.34.35"),
		),
		ril.MeasureMultipleEndpoints(
			ril.Compose(
				ril.MakeEndpointsForPort(443),
				ril.NewEndpointPipeline(
					ril.TCPConnect(),
					ril.TLSHandshake(
					/*
						ril.TLSHandshakeOptionALPN("h2", "http/1.1"),
						ril.TLSHandshakeOptionSNI("www.example.com"),
						ril.TLSHandshakeOptionSkipVerify(true),
						ril.TLSHandshakeOptionX509Certs(
							"-----BEGIN CERTIFICATE-----\\nrwLV\n-----END CERTIFICATE-----",
							"-----BEGIN CERTIFICATE-----\\n9qfgAJizyy3kqM4lOwBH\n-----END CERTIFICATE-----",
						),
					*/
					),
					ril.HTTPTransaction(),
				),
			),
			ril.Compose(
				ril.MakeEndpointsForPort(80),
				ril.NewEndpointPipeline(
					ril.TCPConnect(),
					ril.HTTPTransaction(),
				),
			),
			ril.Compose(
				ril.MakeEndpointsForPort(443),
				ril.NewEndpointPipeline(
					ril.QUICHandshake(
					/*
						ril.QUICHandshakeOptionALPN("h3"),
						ril.QUICHandshakeOptionSNI("www.example.com"),
						ril.QUICHandshakeOptionSkipVerify(true),
						ril.QUICHandshakeOptionX509Certs(
							"-----BEGIN CERTIFICATE-----\\nrwLV\n-----END CERTIFICATE-----",
							"-----BEGIN CERTIFICATE-----\\n9qfgAJizyy3kqM4lOwBH\n-----END CERTIFICATE-----",
						),
					*/
					),
					ril.HTTPTransaction(),
				),
			),
		),
	)

	rawAst := runtimex.Try1(json.Marshal(ril.ExportASTNode(pipeline)))
	fmt.Printf("%s\n", string(rawAst))

	var rootNode ric.ASTNode
	runtimex.Try0(json.Unmarshal(rawAst, &rootNode))

	compiler := ric.NewCompiler()

	f0 := runtimex.Try1(compiler.Compile(&rootNode))

	rtx := rix.NewRuntime(rix.RuntimeOptionLogger(log.Log))
	defer rtx.Close()

	result := f0.Apply(context.Background(), rtx, &rix.Void{})
	log.Infof("%T %+v", result, result)

	observations := rtx.ExtractObservations()
	fmt.Printf("%s\n", string(runtimex.Try1(json.Marshal(observations))))
}
